...
This ERC defines an ERC-721 extension for service objects. A service object exposes a service manifest, a service operator, and a payment route consisting of a revenue recipient, payment manifest URI, payment manifest hash, and route nonce.
...
A compliant contract MUST implement ERC-721 and ERC-165.

The ERC-165 interface ID for the following interface is `0xf94c99e5`.
...
`serviceId` is the ERC-721 `tokenId`.
...
ERC-721 is used for ownership because wallets, marketplaces, and indexers already understand ERC-721 transfers and metadata.
...
The service operator is separate from ERC-721 approvals because marketplace approvals are transfer permissions, not service operation permissions.

URI/hash commitments are used instead of onchain endpoint arrays because service metadata can be large and mutable.

## Backwards Compatibility

This ERC is opt-in and does not change ERC-721 behavior. Contracts that do not implement this interface remain ordinary ERC-721 contracts.

Existing service assets can expose this interface directly or through a wrapper that also exposes the required ERC-721 ownership semantics. A later profile may define how the same service semantics are exposed by an ERC-7656-linked service contract.
